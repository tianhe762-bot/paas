...
def test_dedup_by_message_id(conn):
    saved, _ = s.record_items(conn, "u1", [make_item()], "qq", "m1")
    assert len(saved) == 1
    saved2, skipped2 = s.record_items(conn, "u1", [make_item()], "qq", "m1")
    assert saved2 == []
...
def test_dedup_distinct_message_ids(conn):
    saved, _ = s.record_items(conn, "u1", [make_item()], "qq", "m1")
    saved2, _ = s.record_items(conn, "u1", [make_item()], "qq", "m2")
...
def test_debounce_window(conn):
    saved, _ = s.record_items(conn, "u1", [make_item()], "qq", "m1")
    assert saved
    assert s.find_recent_duplicate(conn, "u1", make_item(), window_seconds=30) is True
    conn.execute(
        "UPDATE expenses SET created_at = datetime('now', '-2 minutes') WHERE message_id = 'm1'"
    )
    conn.commit()
    assert s.find_recent_duplicate(conn, "u1", make_item(), window_seconds=30) is False


def test_pending_ttl(conn):
    s.set_pending(conn, "u1", "DUPLICATE_CONFIRM", {"items": []}, ttl_seconds=600)
    assert s.get_pending(conn, "u1") is not None
    s.set_pending(conn, "u1", "DUPLICATE_CONFIRM", {"items": []}, ttl_seconds=-10)
    assert s.get_pending(conn, "u1") is None


def test_account_balance_expense_income(conn):
    s.record_items(conn, "u1", [make_item(2500, "吃饭", account="微信")], "qq", "m-e1")
    s.record_items(
        conn, "u1", [make_item(500000, "工资", account="银行卡", tx_type="income")], "qq", "m-i1"
    )
    balances = {a["name"]: a["balance_cents"] for a in s.account_balances(conn, "u1")}
    assert balances["微信"] == -2500
    assert balances["银行卡"] == 500000
    assert s.total_assets(conn, "u1") == 497500
...
def test_transfer_and_fee_balance(conn):
    ...
    s.record_items(conn, "u1", items, "qq", "m-t1")
    balances = {a["name"]: a["balance_cents"] for a in s.account_balances(conn, "u1")}
...
def test_void_record_updates_balance(conn):
    saved, _ = s.record_items(conn, "u1", [make_item(2500, "吃饭", account="微信")], "qq", "m-v1")
    record_id = conn.execute("SELECT id FROM expenses WHERE message_id='m-v1'").fetchone()["id"]
    assert s.account_balance_by_name(conn, "u1", "微信")["balance_cents"] == -2500
    s.void_record(conn, "u1", record_id)
    assert s.account_balance_by_name(conn, "u1", "微信")["balance_cents"] == 0
    row = conn.execute("SELECT status FROM expenses WHERE id=?", (record_id,)).fetchone()
    assert row["status"] == "voided"


def test_modify_record_updates_balance(conn):
    s.record_items(conn, "u1", [make_item(2500, "吃饭", account="微信")], "qq", "m-m1")
    record_id = conn.execute("SELECT id FROM expenses WHERE message_id='m-m1'").fetchone()["id"]
    s.modify_record(conn, "u1", record_id, 3500)
    assert s.account_balance_by_name(conn, "u1", "微信")["balance_cents"] == -3500


def test_balance_adjustment_not_in_consumption(conn):
    s.create_adjustment(conn, "u1", "微信", -1000, "微信对不上", "qq", "m-bal")
    summary = day_summary(conn, "u1", datetime.date(2026, 8, 28))
    assert summary["total_cents"] == 0
    assert summary["count"] == 0
    assert s.account_balance_by_name(conn, "u1", "微信")["balance_cents"] == -1000
...
def test_yesterday_record_does_not_mark_today_reported(conn):
    ...
    s.record_items(conn, "u1", [item], "qq", "m-y")
    status = conn.execute(
...
def test_period_stats(conn):
    ...
    s.record_items(conn, "u1", [make_item(2500, "吃饭", account="微信")], "qq", "m-s1")
    s.record_items(conn, "u1", [make_item(3000, "打车", day=today, account="微信")], "qq", "m-s2")
    s.record_items(
        conn, "u1", [make_item(5000, "工资", day=today, account="银行卡", tx_type="income")], "qq", "m-s3"
    )
    stats = s.period_stats(conn, "u1", today, today)
    assert stats["expense_cents"] == 5500
    assert stats["income_cents"] == 5000
    assert stats["count"] == 3
    stats_wx = s.period_stats(conn, "u1", today, today, account_name="微信")
...
def test_import_csv_roundtrip(conn):
    ...
    result, items = s.import_file(conn, "u1", "qq", "imp1", "ledger.csv", data)
    assert result.total_rows == 2
    assert result.success_rows == 2
    assert len(items) == 2
    assert items[0].amount_cents == 8650
    result2, _ = s.import_file(conn, "u1", "qq", "imp1", "ledger.csv", data)
